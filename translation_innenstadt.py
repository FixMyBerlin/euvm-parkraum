...
#   passenden OSM-Attributen (für den Innenstadt-Datensatz).                #
#---------------------------------------------------------------------------#
...
layer.setName('output_parking_areas_translated_innenstadt')
QgsProject.instance().addMapLayer(layer, True)

print(time.strftime('%H:%M:%S', time.localtime()), 'Attributübersetzung fertiggestellt.')

# Datenkorrekturen nach Abschluss:
# - (988x) Flächen mit (dauerhaft) restriction=no_parking/no_stopping ggf. entfernen (Achtung: es können Semente dabei sein, wo dies offensichtlich nicht der Fall ist, z.B. mit zusätzlichen maxstay-Angaben)
# - (56x) Flächen mit (dauerhaft) restriction=no_stopping;no_parking ggf. entfernen
